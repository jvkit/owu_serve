...
<section class="card">
  ...
  <div class="metric">
    <div class="bar">
      <div class="bar-fill" style="width: {percent(quota.chat_quota_used || 0, quota.chat_quota_total || 0)}%"></div>
    </div>
    <p>
      已用 <strong>${formatUsd(quota.chat_quota_used || 0)}</strong>
      / 剩余 <strong>${formatUsd(quota.chat_quota_remaining || 0)}</strong>
      / 总额 <strong>${formatUsd(quota.chat_quota_total || 0)}</strong>
    </p>
  </div>
</section>
